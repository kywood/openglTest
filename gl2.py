...
app = FastAPI(title="OpenGL Offscreen Image Server (Vectorized)")
...
def create_offscreen_context(w: int, h: int):
    # 비가시 창 (헤드리스 환경은 xvfb/EGL 필요할 수 있음)
    if not glfw.init():
...
def render_image(width: int, height: int, arrows: int = 100) -> bytes:
    ...
    try:
        ...
        set_safe_line_width(1.0)  # Core 프로필 호환
        ...
        shaft_len = 0.08 + np.random.rand(N).astype(np.float32)*0.10
        head_len  = shaft_len * 0.45
        head_w    = head_len * 0.10  # ← 머리 폭 (얇게)

        # 샤프트 끝점
        qx = px + dx*shaft_len
        qy = py + dy*shaft_len

        # 샤프트 정점 배열 (N * 2 vertices)
        shafts = np.column_stack([px, py, qx, qy]).reshape(-1, 2).astype(np.float32)

        # 머리(삼각형) 정점 배열 (N * 3 vertices)
        nx = -dy; ny = dx
...
if __name__ == "__main__":
    import uvicorn
    uvicorn.run("gl2:app", host="0.0.0.0", port=8000)
